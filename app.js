...
function go(id){document.querySelectorAll(".screen").forEach(s=>{s.classList.toggle("active",s.id===id);if(s.id===id)s.scrollTop=0});document.querySelectorAll(".bottom button").forEach(b=>{const t=b.dataset.target;const active=t===id||(t==="zones"&&["zones","rachis","techniques","player"].includes(id))||(t==="revision"&&["revision","revisionCard"].includes(id))||(t==="home"&&["quiz","progress"].includes(id));b.classList.toggle("active",active)})}
function toast(message){const el=document.getElementById("toast");el.textContent=message;el.classList.add("show");clearTimeout(window.toastTimer);window.toastTimer=setTimeout(()=>el.classList.remove("show"),1500)}
function openSubzone(subzone){currentSubzone=subzone;currentBack="rachis";document.getElementById("techTitle").textContent=subzone;renderTechniques();go("techniques")}
function openZone(zone){currentSubzone=zone;currentBack="zones";document.getElementById("techTitle").textContent=zone;renderTechniques();go("techniques")}
function renderTechniques(){const query=(document.getElementById("searchInput")?.value||"").toLowerCase();let items=techniques.filter(t=>t.sub===currentSubzone&&t.title.toLowerCase().includes(query));if(currentSubzone==="Membres supérieurs"){items=[{id:9,title:"Correction de l’épaule en énergie musculaire",level:"Niveau intermédiaire",status:"non vue",percent:0,icon:"▷"},{id:10,title:"Technique du coude en énergie musculaire",level:"Niveau intermédiaire",status:"non vue",percent:0,icon:"↻"}]};if(currentSubzone==="Membres inférieurs"){items=[{id:11,title:"Technique du genou en chaîne fermée",level:"Niveau intermédiaire",status:"non vue",percent:0,icon:"▷"},{id:12,title:"Technique de cheville en énergie musculaire",level:"Niveau débutant",status:"non vue",percent:0,icon:"↻"}]};const list=document.getElementById("techList");if(!list)return;list.innerHTML=items.map((t,i)=>`<button class="row tech ${i===0?'featured':''}" onclick="openTechnique(${t.id})"><div class="tech-icon">${t.icon}</div><div><h3>${t.title}</h3><small>${t.level}</small></div><div class="state">${stateLabel(t)}</div></button>`).join("")}
